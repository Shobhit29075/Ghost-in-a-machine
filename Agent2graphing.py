...
class Cell:
    ...
    def draw_ghost(self):
        x,y=self.x*TILE,self.y*TILE
        sc.blit(ghost, (x, y))
### Check neighbors and exclude those that have ghosts in it
...
######To store all possible ghost spawn location then spawn n ghosts randomly among these locations
        ...
        ghost_locations_for_valid=ghost_locations.copy()
        ghost_locations=[]
        for i in ghost_locations:
        ...
        if(flag==1):
            continue
        ghost_locations=ghost_locations_for_valid.copy()
        ...
        while run:
            ...
            ####without this it takes about 120min for 300 simulations..90 sec with this
            if path=="no possible path":
                path=astarsearch(current_cell)
            elif len(list(set(path) & set(ghost_locations)))!=0:
                path=astarsearch(current_cell)
            if path=="no possible path":
